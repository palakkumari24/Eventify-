...
export default function BookingForm({ initialEvent }) {
  ...
  useEffect(() => {
    if (initialEvent) {
      setFormData((f) => ({ ...f, venue: initialEvent.venue || '', date: initialEvent.date || '' }))
    }
  }, [initialEvent])
  ...
  const canProceed = () => {
    ...
    if (currentStep === 3) return formData.guests && formData.name && formData.email
    return false
  }
  ...
  const handleNext = async () => {
    ...
    try {
      const payload = {
        event: initialEvent?._id || initialEvent?.id || null,
        date: formData.date,
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-3xl p-8 md:p-12 shadow-xl border border-white/50">
        ...
        {currentStep === 3 && (
          <div className="animate-fade-in">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">Guest details</h3>
            <input
              type="number"
              min="10"
              max="1000"
              ...
              className="w-full px-6 py-4 text-lg border-2 border-gray-200 rounded-xl focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 outline-none transition-all mb-4"
            />
            <input
              type="text"
